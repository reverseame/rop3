...
A raw gadget lists its instruction mnemonics, their operands, and the concrete registers it writes (`dst`) and reads (`src`) — the last two feed the assembler's side-effect tracking, so a raw gadget composes with the rest of the chain like any operation. It is matched verbatim (a memory operand is written `[reg]`), so raw gadgets are architecture specific and are not translated across architectures. A raw gadget carries **no** ret/branch terminator requirement: it is matched wherever the exact instruction sequence appears in the binary, whether or not it ends in a control-flow transfer — so a chain-ending `syscall`/`svc 0`/`ecall` with nothing reachable after it is expressible (and may sit anywhere in the chain). Include a terminator among the mnemonics if you want one (`raw([pop, ret], [rdi], [rdi], [])`). A raw gadget is located by assembling the pattern (with [Keystone](https://www.keystone-engine.org/), an optional dependency) and searching the binary for those exact bytes, taking the first match — so it finds the assembled encoding of the sequence; a semantically equivalent but differently encoded copy is not matched. When Keystone is unavailable (not installed, or no backend for the architecture such as RISC-V), rop3 falls back to disassembling to locate the pattern, so raw gadgets still work everywhere. By default every operand belongs to the first instruction; to spread operands over several instructions, wrap each instruction's operands in parentheses:
